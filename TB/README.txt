...
| `requirements.txt`            | install this Requirements.txt to run the code soomtly             |
...
1. Place ` Cheat_X-Ray.png` (X-ray image) and `TB_Mask.tif` (mask) in the project directory.
2. Run the script:
...
6. **3D Generation**: The profile is rotated in 360° using meshgrids.
7. **Color Mapping**:
   - **Gray**: Base from X-ray intensity.
   - **Red**: Areas marked as TB-affected by the mask.
8. **Visualization**: The 3D model is rendered using Plotly with interactive rotation.
